...
export class ApiProcessCacheWebsocket implements ApiSocketConnection {
  ...
  initialize = async (ws: expressWs.Instance): Promise<void> => {
    ...
    ws.app.ws('/api/process/:identifier/cache', (ws, req) => {
      ...
      this.logger.trace('Process cache state websocket connected', identifier, this.stateWebsocketsByIdentifier[identifier].length);

      ws.on('close', () => {
        this.stateWebsocketsByIdentifier[identifier] = this.stateWebsocketsByIdentifier[identifier].filter((socket) => socket !== ws);
        this.logger.trace('Process cache state websocket disconnected', identifier);
      });
    });
  };
