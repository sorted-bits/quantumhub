import { FanAttribute, FanDevice } from "quantumhub-sdk";
import { Hub } from "../../hub";
...
export class FanDescription extends BaseAttributeDescription {
    ...
    constructor(hub: Hub, provider: PackageProvider, attribute: FanAttribute) {
        ...
        this.state_value_template = `{{ value_json.${attribute.key}.state }}`;

        if (attribute.has_speed_control) {
            this.percentage_command_template = `${this.stateTopic}/${attribute.key}/speed/set`;
            this.percentage_state_template = this.stateTopic;
            this.percentage_value_template = `{{ value_json.${attribute.key}.percentage }}`;
        }

        if (attribute.preset_modes) {
            this.preset_modes = attribute.preset_modes;
            this.preset_mode_command_topic = `${this.stateTopic}/${attribute.key}/preset_mode/set`;
            this.preset_mode_state_topic = this.stateTopic;
            this.preset_mode_value_template = `{{ value_json.${attribute.key}.preset_mode}}`;
        }

        if (attribute.has_oscillation) {
            this.oscillation_command_topic = `${this.stateTopic}/${attribute.key}/oscillation/set`;
            this.oscillation_state_topic = this.stateTopic;
            this.oscillation_value_template = `{{ value_json.${attribute.key}.oscillation }}`;
        }

        if (attribute.has_direction) {
            this.direction_command_topic = `${this.stateTopic}/${attribute.key}/direction/set`;
            this.direction_state_topic = this.stateTopic;
            this.direction_value_template = `{{ value_json.${attribute.key}.direction }}`;
        }
